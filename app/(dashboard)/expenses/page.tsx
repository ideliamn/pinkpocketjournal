...
export default function ExpensesPage() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      if (!meRes.ok || meResult.code === 0) return;
      ...
      const params = new URLSearchParams({
        ...
        planId: String(activePlan.plan_id),
        page: String(page),
        limit: String(limit),
      });
      ...
      const [expenseRes, summaryRes, categoryRes] = await Promise.all([
        fetch(`/api/expense?${params.toString()}`),
        fetch(`/api/expense/summary?planId=${activePlan.plan_id}`),
        fetch(`/api/category?userId=${currentUser.id}&type=expense`),
      ]);
      ...
      setTotalPages(expenseResult.totalPages || 0);
